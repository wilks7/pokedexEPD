...
# Test function to build and print the query URL
def test_build_query():
    pokedex = 25  # Example Pokemon number (Pikachu)
    generation = 1
    query = _build_query(pokedex, generation)
    print("Generated GraphQL query:")
    print(query)

# Test function to print the endpoint URL along with the query and fetch result
def test_fetch_pokemon():
    pokedex = 25  # Example Pokemon number (Pikachu)
    generation = 1
    pokemon = fetch_pokemon(pokedex, generation)
    if pokemon:
